...
EventBus基于观察者模式中的发布-订阅(Publish-Subscribe)模式，但与传统的观察者模式不同，在传
统的观察者模式下，我们更多是处于订阅者的身份，数据的发布大多数情况下我们是不可控的，我们一般只能
触发数据开始发送，而EventBus则将发布(post)和订阅的能力都交由我们自己处理；
